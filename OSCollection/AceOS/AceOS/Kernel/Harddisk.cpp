...
BOOLEAN IsDeviceDataReady(int DeviceController,BYTE WaitUpToms=0,BOOLEAN CheckDataRequest=True)
	...
	do
		...
		Time2=GetTimerTick();
		}while ( ( Time2-Time1 ) < WaitUpToms );  /* Wait for data ready */
	return False;
	}
/*
  ---------------------------------------------------------
...
BOOLEAN IsDeviceControllerBusy(int DeviceController, BYTE WaitUpToms=0)
	{UINT32 Time1,Time2;
	Time1=GetTimerTick();
	do	{UINT16 PortID=IDE_Con_IOBases[DeviceController][0]+IDE_CB_STATUS;
		BYTE Status=InPortByte(PortID);
...
void HardDiskHandler::Initialize()
	...
	for(BYTE DeviceController=0;DeviceController<IDE_MAX_CONTROLLER;DeviceController++)
		...
		for (BYTE Device=0;Device<2;Device++)         // loop for master and slave disks
			...
			struct __HDDInfo * newHDD=(struct __HDDInfo *)AllocHeap(sizeof(struct __HDDInfo));
			if ( newHDD == NULL )
...
__HDDInfo * HardDiskHandler::GetHDDInfo(BYTE * DPF)
	{__HDDInfo * getHDD, * retHDD=(struct __HDDInfo *)AllocHeap(sizeof(struct __HDDInfo));
