...
Collection <struct PCIDeviceDetails *> __SysPCIDevices;
		...
		for(Device=0;Device<32;Device++)
			for(Function=0;Function<8;Function++)
				...
				if (!( PCICfg.VendorID==0xFFFF || PCICfg.DeviceID==0xFFFFFFFF ))
					{newPCIDev = ( struct PCIDeviceDetails * ) AllocHeap(sizeof( struct PCIDeviceDetails ));
					newPCIDev->Bus = Bus;
					...
					__SysPCIDevices.Add(newPCIDev);
					TotDev++;
					}
				}
	return TotDev;
	}
extern "C" void EnumeratePCIDevices( BYTE (*CallBackFn) (struct PCIConfigurationSpace * )  )
	{for(UINT16 i=0; i<__SysPCIDevices.Count(); i++ )
		{struct PCIConfigurationSpace * ptrPCIDet;
		ptrPCIDet = &__SysPCIDevices.Item(i)->PCIConfDetails;
		if ( ptrPCIDet != NULL )
			{BYTE (*CallBackFun)(struct PCIConfigurationSpace * ) = (BYTE (*)(struct PCIConfigurationSpace * )) ((UINT32)CallBackFn-0x19000);
			CallBackFun(ptrPCIDet);
			}
		}
	}
