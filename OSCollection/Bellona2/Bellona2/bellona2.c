...
void CreateGDT()
{
	...
	gdtr.wSize = MAX_GDT * sizeof(DescriptorStt);
	gdtr.dwAddr = (ULONG)&gdt[0];
...
void bellona2_main2(unsigned long magic, unsigned long addr, unsigned int imageBase);
//_declspec(naked) void bellona2_main()
void bellona2_main(unsigned long magic, unsigned long addr, unsigned int imageBase)
{
	_asm
	{
		MOV EAX, [magic]
		MOV EBX, [addr]
		MOV ECX, [imageBase]
		MOV     ESP, KERNEL_INIT_STACK_BASE; //스택 설정
		MOV     EBP, ESP
		PUSH    0; //플래그 레지스터 초기화
		POPF
		PUSH ECX
...
void bellona2_main2(unsigned long magic, unsigned long addr, unsigned int imageBase)
{
	...
	__asm
	{
		...
		MOV     ESP, KERNEL_INIT_STACK_BASE
		PUSH	0; //파라메터
		PUSH	0; //EBP
		PUSH    0x200; EFLAGS
		PUSH    0x08; CS
