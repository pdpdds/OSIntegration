...
void CrtClearScreen()
{
	...
	pVideoBuf = (unsigned char *)VIDEO_BUFFER_ADDRESS;
	while (pVideoBuf < (unsigned char *)(VIDEO_BUFFER_ADDRESS + VIDEO_BUFFER_SIZE)) {
		*pVideoBuf++ = ' ';
...
void HariMain(unsigned long magic, unsigned long addr, unsigned int imageBase)
{
	...
	*((int *) 0x0fec) = (int) &fifo;
	...
	init_palette();
	multiboot_info* mb_info = (multiboot_info*)addr;
	shtctl = shtctl_init(memman, mb_info->framebuffer_addr, mb_info->framebuffer_width, mb_info->framebuffer_height);
	//CrtClearScreen();
	//for (;;);
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	buf_back  = (unsigned char *) memman_alloc_4k(memman, mb_info->framebuffer_width * mb_info->framebuffer_height);
	sheet_setbuf(sht_back, buf_back, mb_info->framebuffer_width, mb_info->framebuffer_height, -1); /* 투명색없음 */


	init_screen8(buf_back, mb_info->framebuffer_width, mb_info->framebuffer_height);
	...
	mx = (mb_info->framebuffer_width - 16) / 2; /* 화면 중앙이 되도록 좌표 계산 */
	my = (mb_info->framebuffer_height - 28 - 16) / 2;
