...
kInPortByte:
    ...
    mov rdx, rcx    ; RDX 레지스터에 파라미터 1(포트 번호)를 저장
...
kLoadGDTR:
    lgdt [ rcx ]    ; 파라미터 1(GDTR의 어드레스)를 프로세서에 로드하여
                    ; GDT 테이블을 설정
    ret
...
kLoadTR:	
    ltr cx          ; 파라미터 1(TSS 세그먼트 디스크립터의 오프셋)을 프로세서에
                    ; 설정하여 TSS 세그먼트를 로드
    ret
...
kLoadIDTR:
    lidt [ rcx ]    ; 파라미터 1(IDTR의 어드레스)을 프로세서에 로드하여
                    ; IDT 테이블을 설정
    ret
...
kWriteMSR:
    push rdi        ; RDX 레지스터부터 RCX 레지스터까지 값을 스택에 보관
    push rax
    push rsi
    
    mov rcx, rcx    ; 첫 번째 파라미터의 MSR 어드레스를 RCX 레지스터에 저장
    mov rax, R8    ; 세 번째 파라미터의 qwRAX 값을 RAX 레지스터에 저장
    mov rdx, rdx    ; 두 번째 파라미터의 qwRDX 값을 RDX 레지스터에 저장
    wrmsr           ; RCX 레지스터에 저장된 MSR에 RDX 레지스터(상위 32비트)와
                    ; RAX 레지스터(하위 32비트)를 저장

    pop rsi         ; RDX 레지스터부터 RCX 레지스터까지 값을 스택에서 복원
    pop rax
    pop rdi
    ret             ; 함수를 호출한 다음 코드의 위치로 복귀
...
kSwitchContext:
    ...
    cmp rcx, 0      ; Current Context가 NULL이면 콘텍스트 복원으로 바로 이동
    je .LoadContext
    ...
    mov qword[ rcx + ( 22 * 8 ) ], rax  ; 제외한 값으로 저장
    ...
    mov qword[ rcx + ( 19 * 8 ) ], rax  ; 다음 콘텍스트 복원 시에 이 함수를 호출한 
                                        ; 위치로 이동하게 함
...
kSaveFPUContext:
    fxsave  [ rcx ]     ; 첫 번째 파라미터로 전달된 버퍼에 FPU 레지스터를 저장
    ret
    
; FPU 관련 레지스터를 콘텍스트 버퍼에서 복원
;   PARAM: Buffer Address
kLoadFPUContext:
    fxrstor [ rcx ]     ; 첫 번째 파라미터로 전달된 버퍼에서 FPU 레지스터를 복원
    ret   
